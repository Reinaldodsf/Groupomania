...
class PostsModels {
    ...
    getAllPosts(){
        let sql = "SELECT posts.postId, posts.userId, posts.title, posts.content, DATE_FORMAT(DATE(posts.date), '%d/%m/%Y') AS date, TIME(posts.date) AS time, posts.likes, users.lastName, users.firstName FROM posts JOIN users ON posts.userId = users.userId ORDER BY posts.date DESC";
    ...
    updatePost(sqlInserts1, sqlInserts2){
        let sql1 = 'SELECT * FROM posts where postId = ?';
        sql1 = mysql.format(sql1, sqlInserts1);
        return new Promise((resolve) =>{
            connectdb.query(sql1, function (err, result, fields){
                if (err) throw err;
                if(sqlInserts2[3] == result[0].userId){
                    let sql2 = 'UPDATE posts SET title = ?, content = ? WHERE postId = ? AND userId = ?';
        ...
        return new Promise((resolve, reject) =>{
            connectdb.query(sql1, function (err, result, fields){
                ...
                if(sqlInserts2[1] == result[0].userId){
                    let sql2 = 'DELETE FROM posts WHERE postId = ? AND userId = ?';
                    sql2 = mysql.format(sql2, sqlInserts2);
                    connectdb.query(sql2, function (err, result, fields){
                        if (err) throw err;
    ...
    getComments(sqlInserts){
        let sql = "SELECT comments.comContent, DATE_FORMAT(comments.date, '%d/%m/%Y à %H:%i:%s') AS date, comments.commentsId, comments.userId, users.firstName, users.lastName FROM comments JOIN users on comments.userId = users.userId WHERE postId = ? ORDER BY date";
        sql = mysql.format(sql, sqlInserts);
        return new Promise((resolve) =>{
            connectdb.query(sql, function (err, result, fields){
                if (err) throw err;
    ...
    updateComment(sqlInserts1, sqlInserts2){
        let sql1 = 'SELECT * FROM comments where commentsId = ?';
        sql1 = mysql.format(sql1, sqlInserts1);
        return new Promise((resolve) =>{
            connectdb.query(sql1, function (err, result, fields){
                if (err) throw err;
                if(sqlInserts2[2] == result[0].userId){
                    let sql2 = 'UPDATE comments SET comContent = ? WHERE commentsId = ? AND userId = ?';
                    sql2 = mysql.format(sql2, sqlInserts2);
                    connectdb.query(sql2, function (err, result, fields){
                        if (err) throw err;
    ...
    deleteComment(sqlInserts1, sqlInserts2){
        ...
        return new Promise((resolve, reject) =>{
            connectdb.query(sql1, function (err, result, fields){
                if (err) throw err;
                if(sqlInserts2[1] == result[0].userId){
                    let sql2 = 'DELETE FROM comments WHERE commentsId = ? AND userId = ?';
                    sql2 = mysql.format(sql2, sqlInserts2);
                    connectdb.query(sql2, function (err, result, fields){
                        if (err) throw err;
    ...
    postLike(sqlInserts1, sqlInserts2, liked){
        ...
        sql1 = mysql.format(sql1, sqlInserts1);
        let sql2 = 'UPDATE posts SET likes = ? WHERE postId = ?';
        sql2 = mysql.format(sql2, sqlInserts2);
